/**
 * Rocket Logistic - Bulgarian Page JavaScript
 * Navigation, form handling, and interactions
 */
...
(function() {
    ...
    if (contactForm) {
        contactForm.addEventListener('submit', function(e) {
            ...
            if (message.length < 10) {
                showError('message', 'Моля въведете съобщение (мин. 10 символа)');
                isValid = false;
            } else {
                clearError('message');
            }
            ...
            if (isValid) {
                ...
                .then(data => {
                    if (data.success) {
                        ...
                        const errorMessage = data.error_bg || data.error || 'Възникна грешка. Моля, опитайте отново.';
                        showFormOverlay('error', errorMessage);
                    }
                })
                .catch(error => {
                    console.error('Error:', error);
                    showFormOverlay('error', 'Мрежова грешка. Моля, проверете връзката си и опитайте отново.');
                })
                .finally(() => {
    ...
    function showFormOverlay(type, customMessage) {
        ...
        const title = isSuccess ? 'Съобщението е изпратено!' : 'Грешка';
        const message = isSuccess
            ? 'Благодарим ви! Вашето съобщение е изпратено. Ще се свържем с вас скоро.'
            : customMessage;
